...
class _GameViewState extends ConsumerState<GameView> {
  ...
  void initState() {
    ...
    _subscription = ref.listenManual<PuzzleState>(puzzleProvider, (previous, next) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Level ${puzzleState.level}'),
        leading: IconButton(
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            final availableHeight = constraints.maxHeight.isFinite ? constraints.maxHeight : constraints.maxWidth;
...
class _SidePanel extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            CounterWidget(label: 'Moves', value: state.moves.toString()),
            CounterWidget(label: 'Time', value: state.formattedTime),
          ],
        ),
        const SizedBox(height: 16),
        Text('Leaderboards', style: Theme.of(context).textTheme.titleMedium),
        const SizedBox(height: 8),
        _buildLeaderboard(context),
        const SizedBox(height: 16),
        AppButton(label: 'Restart', onPressed: onRestart),
        const SizedBox(height: 12),
        AppButton(label: 'Next Level', onPressed: onNextLevel),
      ],
    );
  }
  ...
  Widget _buildLeaderboard(BuildContext context) {
    return leaderboard.when(
      data: (entries) {
        if (entries.isEmpty) {
          return const Text('Play to create your first score!');
        }
        return Column(
          children: entries
              .map(
                (entry) => ListTile(
                  ...
                  title: Text('${entry.name} - ${entry.moves} moves'),
      ...
      error: (error, _) => Text('Unable to load leaderboard: $error'),
    );
  }
